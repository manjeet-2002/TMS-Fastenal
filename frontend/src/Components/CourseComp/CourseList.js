...
const CourseList = (props) => {
  ...
  return (
    <>
      {props.courseType === props.options[0] && (
        <div className="course__list">
          {courseStateList.pastCourses.map((course, index) => {
            return <CourseItem key={index} course={course} />;
          })}
        </div>
      )}
      {props.courseType === props.options[1] && (
        <div className="course__list">
          {courseStateList.currentCourses.map((course, index) => {
            return <CourseItem key={index} course={course} />;
          })}
        </div>
      )}
      {props.courseType === props.options[2] && (
        <div className="course__list">
          {courseStateList.upcomingCourses.map((course, index) => {
            return <CourseItem key={index} course={course} />;
          })}
        </div>
      )}
